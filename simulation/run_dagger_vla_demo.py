"""Step 8 single-episode demo: the Temporal Dense VLA + DAgger fine-tuned
checkpoint controls the robot in closed loop.

Usage:
    python -m simulation.run_dagger_vla_demo --checkpoint outputs/training/temporal_dagger_run_001/best.pt

Same architecture/policy class as Step 7 (``TemporalDenseVLAPolicy`` --
README "DAgger policy class") -- only the checkpoint's weights differ.
Reuses ``evaluation.closed_loop.run_closed_loop_episode`` unmodified; no
scripted/corrective expert is instantiated anywhere in this module (README
"No runtime expert fallback").
"""
...
def main(argv: list = None) -> None:
    ...
    frames_dir = OUTPUT_DIR / "dagger_vla_demo_frames"
    if args.save_frames:
        frames_dir.mkdir(parents=True, exist_ok=True)

    device = resolve_device(args.device)
    print("Controller: Temporal Dense VLA + DAgger")
    ...
    with SimulationEnvironment() as env:
        ...
        save_rgb(initial_obs.rgb, OUTPUT_DIR / "dagger_vla_demo_initial.png")
        print(f"Saved: {OUTPUT_DIR / 'dagger_vla_demo_initial.png'}")
        ...
        save_rgb(final_obs.rgb, OUTPUT_DIR / "dagger_vla_demo_final.png")
        print(f"Saved: {OUTPUT_DIR / 'dagger_vla_demo_final.png'}")
    ...
    print()
    print(
        "Note: this is a single closed-loop rollout under the Temporal Dense VLA + DAgger "
        "policy. See README for the Temporal-vs-Temporal+DAgger comparison."
    )
