"""Step 8 closed-loop evaluation: N episodes under the Temporal Dense VLA +
DAgger fine-tuned policy.
...
Identical protocol to ``simulation/evaluate_temporal_vla_closed_loop.py``
(same seed/episodes/xy-randomization convention -> identical cube-offset
sequence, so Dense/MoE/Temporal/Temporal+DAgger are evaluated on the same
initial cube positions) and the exact same ``TemporalDenseVLAPolicy`` +
``run_closed_loop_episode`` runtime path -- no scripted/corrective expert
is imported here (README "No runtime expert fallback"). Writes to a
SEPARATE output directory so Step 7's frozen Temporal baseline outputs are
never touched (README "Preserve all existing baselines").
"""
...
DEFAULT_OUTPUT = "outputs/evaluation/temporal_dagger_vla_closed_loop"
...
def main(argv: list = None) -> dict:
    ...
    print()
    print("Temporal Dense VLA + DAgger Closed-Loop Evaluation")
    print()
