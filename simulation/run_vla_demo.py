"""Step 5 single-episode demo: the trained Dense VLA controls the robot in closed loop.

Usage:
    python -m simulation.run_vla_demo --checkpoint outputs/training/dense_vla_run_001/best.pt

Controller: Dense VLA Policy -- NOT the scripted expert. Every action in
this rollout comes from ``DenseVLAPolicy.predict()``; ``ScriptedController``
is never imported here. See ``evaluation/closed_loop.py`` for the shared
rollout implementation this demo and ``evaluate_vla_closed_loop.py`` both
use.
"""
...
FRAME_INTERVAL = 20  # save a periodic frame every N ticks when --save-frames is set
...
def main(argv: list = None) -> None:
    ...
    frames_dir = OUTPUT_DIR / "vla_demo_frames"
    if args.save_frames:
    ...
    with SimulationEnvironment() as env:
        ...
        save_rgb(initial_obs.rgb, OUTPUT_DIR / "vla_demo_initial.png")
        print(f"Saved: {OUTPUT_DIR / 'vla_demo_initial.png'}")
        ...
        result = run_closed_loop_episode(
            ...
            on_step=maybe_save_frame,
        )

        final_obs = env.get_observation()
        save_rgb(final_obs.rgb, OUTPUT_DIR / "vla_demo_final.png")
        print(f"Saved: {OUTPUT_DIR / 'vla_demo_final.png'}")
    ...
    print()
    print(
        "Note: this is a single closed-loop rollout under the learned Dense VLA "
        "policy, not the scripted expert. See README for offline-vs-closed-loop distinction."
    )
